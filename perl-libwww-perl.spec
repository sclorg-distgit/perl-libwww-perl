%{?scl:%scl_package perl-libwww-perl}
...
BuildRequires:  %{?scl_prefix}perl(Encode) >= 2.12
BuildRequires:  %{?scl_prefix}perl(Encode::Locale)
...
BuildRequires:  %{?scl_prefix}perl(HTML::HeadParser)
BuildRequires:  %{?scl_prefix}perl(HTTP::Config)
# HTTP::Cookies 6 not used at tests
BuildRequires:  %{?scl_prefix}perl(HTTP::Date) >= 6
BuildRequires:  %{?scl_prefix}perl(HTTP::Headers::Util)
# HTTP::Negotiate 6 not used at tests
BuildRequires:  %{?scl_prefix}perl(HTTP::Request) >= 6
BuildRequires:  %{?scl_prefix}perl(HTTP::Request::Common) >= 6
...
BuildConflicts: %{?scl_prefix}perl(HTTP::Status) = 6.17
...
BuildRequires:  %{?scl_prefix}perl(Net::HTTP) >= 6.18
...
BuildRequires:  %{?scl_prefix}perl(URI::Escape)
# URI::Heuristic not used at tests
BuildRequires:  %{?scl_prefix}perl(WWW::RobotRules) >= 6
# Optional run-time:
...
BuildRequires:  %{?scl_prefix}perl(HTTP::Daemon) >= 6
...
Requires:       %{?scl_prefix}perl(Authen::NTLM) >= 1.02
Requires:       %{?scl_prefix}perl(Encode) >= 2.12
Requires:       %{?scl_prefix}perl(File::Spec)
Requires:       %{?scl_prefix}perl(File::Listing) >= 6
Requires:       %{?scl_prefix}perl(HTML::Entities)
Requires:       %{?scl_prefix}perl(HTML::HeadParser)
Requires:       %{?scl_prefix}perl(HTTP::Config)
Requires:       %{?scl_prefix}perl(HTTP::Cookies) >= 6
Requires:       %{?scl_prefix}perl(HTTP::Date) >= 6
Requires:       %{?scl_prefix}perl(HTTP::Headers::Util)
Requires:       %{?scl_prefix}perl(HTTP::Negotiate) >= 6
Requires:       %{?scl_prefix}perl(HTTP::Request) >= 6
Requires:       %{?scl_prefix}perl(HTTP::Request::Common) >= 6
...
Requires:       %{?scl_prefix}perl(Net::FTP) >= 2.58
Requires:       %{?scl_prefix}perl(Net::HTTP) >= 6.18
Requires:       %{?scl_prefix}perl(URI) >= 1.10
Requires:       %{?scl_prefix}perl(URI::Escape)
Requires:       %{?scl_prefix}perl(WWW::RobotRules) >= 6
Provides:       %{?scl_prefix}perl(LWP::Debug::TraceHTTP::Socket) = %{version}
Provides:       %{?scl_prefix}perl(LWP::Protocol::http::Socket) = %{version}
Provides:       %{?scl_prefix}perl(LWP::Protocol::http::SocketMethods) = %{version}
...
%global __requires_exclude %{?__requires_exclude:%{__requires_exclude}|}^%{?scl_prefix}perl\\((Authen::NTLM|Encode|File::Listing|HTTP::Date|HTTP::Negotiate|HTTP::Request|HTTP::Response|HTTP::Status|LWP::MediaTypes|MIME::Base64|Net::FTP|Net::HTTP|URI|WWW::RobotRules)\\)$
...
%{?scl:scl enable %{scl} '}perl Makefile.PL INSTALLDIRS=vendor NO_PACKLIST=1 NO_PERLLOCAL=1 --aliases < /dev/null && %{make_build}%{?scl:'}
